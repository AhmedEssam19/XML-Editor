...
def XML2json(tree):
    str = RefStr("{\n")
    square = False
    dfs(tree.get_root(), 1, str, square, False, False)
    str += "\n}"
    print(str)
    return str


def dfs(node, depth, str, square, is_last, comma):
    open_bracket = False
    ...
    if not node.is_printed:
        str += f'"{node.tag}"'
        if node.is_leaf():
            if comma or node.my_parent_comma:
                str += ","
            str += "\n"
    ...
    for i in reversed(range(len(node.get_children()))):
        ...
        if i != 0:
            if node.get_children()[i].get_children()[0].is_leaf():
                node.get_children()[i].get_children()[0].my_parent_comma = True
            comma = True
        ...
        dfs(node.get_children()[i], depth + 1, str, square, is_last, comma)

    if open_bracket:
        if depth > 1:
            str += f'{"  " * depth}}}'
            if comma or node.my_parent_comma or not is_last:
                str += ","
            str += "\n"
        else:
            str += f'{"  " * depth}}}'
